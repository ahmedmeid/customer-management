...
@RestController
@RequestMapping("/api/customer-management-kafka")
public class CustomerManagementKafkaResource {

    private final Logger log = LoggerFactory.getLogger(CustomerManagementKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public CustomerManagementKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
